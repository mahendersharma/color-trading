...
const NumberingButtons = ({ onRemove, onSelect }) => {
  const [selectedValue, setSelectedValue] = useState(null);

  const handleAutoSelectClick = () => {
    const randomValue = generateRandomValue();
    setSelectedValue(randomValue);
    onSelect(randomValue); // Pass the selected value directly
  };

  const handleButtonClick = (value) => {
    const updatedValue = selectedValue === value ? null : value; // Toggle selection
    setSelectedValue(updatedValue);
    onSelect(updatedValue); // Pass the selected value directly
  };
  ...
  const generateRandomValue = () => {
    return Math.floor(Math.random() * 39) + 1; // Generate random number between 1 and 39
  };
